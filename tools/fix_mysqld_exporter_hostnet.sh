# /home/melynxis/solace/tools/fix_mysqld_exporter_hostnet.sh
#!/usr/bin/env bash
set -euo pipefail
...
# 1) Switch mysqld_exporter to host network and use 127.0.0.1:3306
python3 - "$FILE" <<'PY'
import sys,re
p=sys.argv[1]; t=open(p).read()
# Ensure mysqld_exporter block exists
m=re.search(r'(\n  mysqld_exporter:\n(?:.*\n)*?)(?=\n  \w|$)', t)
if not m: 
    print("mysqld_exporter block not found", file=sys.stderr); sys.exit(1)
blk=m.group(1)

# Remove "ports:" mapping (host network doesn't need it)
blk=re.sub(r'\n\s+ports:\n(?:\s+-\s*".*"\n)+', '\n', blk)

# Add/ensure network_mode: host
if 'network_mode:' not in blk:
    blk = re.sub(r'(\n\s+restart:.*\n)', r'\1    network_mode: host\n', blk)
else:
    blk = re.sub(r'network_mode:.*', 'network_mode: host', blk)

# Ensure DATA_SOURCE_NAME references host MySQL
blk=re.sub(
...
echo "Restarting exporters stack..."
...
curl -fsS http://127.0.0.1:9104/metrics | head -n 10 || {
  echo "mysqld_exporter still not responding. View logs with:"
